...
?>
    ...
    //echo '<br>';
    //echo 'COOKIE: '.$_COOKIE['upay_token'];
    //echo '<br>';
    ...
    $bank_token = !empty($_SESSION['upay_token'])?$_SESSION['upay_token']:$_COOKIE['upay_token'];

    global $wpdb, $PasswordHash, $current_user, $user_ID;
    
    $bank_tokens_table_name = $wpdb->prefix . "bank_tokens";
    $bank_tokens_retrieve_data = $wpdb->get_row("SELECT * FROM $bank_tokens_table_name WHERE bank_name = 'upay' AND bank_token = '".$bank_token."'");
    
    if (!empty($_REQUEST['status']) && $_REQUEST['status'] == 'success') {
        $header = array(
            'Content-Type:application/json',
            'Authorization:UPAY ' . $bank_token
        );
        ...
        if(!empty($return_var) && $return_var->status == 'success'){
            ...
            elseif(trim($bank_tokens_retrieve_data->payment_for) == 'add vehicle'){
                ...
                if (empty($recharge_retrieve_data->id)) {
                    ...
                    $actual_trip_no = floor($tellamount / $tariff_retrieve_data->toll_rates);
