...
function get_mobile_pin()
{
	...
	if(!empty($_POST['mobile_no'])){
		...
		if(strlen($mobileno)!= 11 && $mobileno != '')
            ...
            $data = [
                "api_key" => "C20088266543cf3ede5759.80350461",
                "type" => "text",
                "contacts" => $_POST['mobile_no'],
                "senderid" => "8809601011377",
                "msg" => $message,
            ];
            $ch = curl_init();
    ...
    $url ="http://orionsms.dhrubosoft.com/ssem_api/send_sms_api";

    $data=array
    (
        "api_key"       => "1-cDuYjex1699858438QFsDi2M",
...
function CityProcessRequest($curl_post_data,$service_url,$proxy,$proxyauth)
{
    ...
    $certfile       = $upload_path.'/city/mmhf.crt';
    $keyfile        = $upload_path.'/city/mmhf.key';
    $ch = curl_init();
...
function addtrip_upay(){
    ...
    $user_data = array("trip_no"=>$_POST['trip_no'], "user_id"=>$_POST['user_id'], "gea_subscription_id"=>$_POST['gea_subscription_id'], "invoice_id"=>$invoice_id, "vehicle_type"=>$_POST['vehicle_type'], "amount"=>$_POST['amount']);
...
function addtrip_city(){
    ...
    $user_data = array("trip_no"=>$_POST['trip_no'], "user_id"=>$_POST['user_id'], "gea_subscription_id"=>$_POST['gea_subscription_id'], "invoice_id"=>$_POST['invoice_id'], "vehicle_type"=>$_POST['vehicle_type'], "amount"=>$_POST['amount']);
